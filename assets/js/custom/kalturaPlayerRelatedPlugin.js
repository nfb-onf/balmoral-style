// Modified from https://github.com/kaltura/mwEmbed/blob/2.50-20-Nov/modules/KalturaSupport/components/related/related.js

mw.kalturaPluginWrapper(function() {

    mw.PluginManager.add( 'related-plugin', mw.KBaseScreen.extend({
        ...
        getItemsData: function( callback ){
            ...
            if( !this.templateData ){
                ...
                var related_names_by_id = this.getConfig( 'related_names_by_id' );

                if( related_names_by_id ) {
                    var ids_string = Object.keys(related_names_by_id).join(",");
                    return this.getEntriesFromList( ids_string, function( fetched_entries ) {
                        var formatted_entries = fetched_entries.map( function(e) {
                            var ne = e;
                            ne.name = related_names_by_id[e.id]
                            return ne;
                        });
                        _this.updateTemplateData( formatted_entries );
                        callback();
                    });
                }
