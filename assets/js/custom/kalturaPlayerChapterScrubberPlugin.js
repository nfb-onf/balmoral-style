// Adapted from
// https://github.com/kaltura/mwEmbed/blob/v2.45.1/modules/KalturaSupport/components/scrubber.js
//
// This plugin powers the video timeline (playhead, thumbnail preview, start and end of segment).


mw.kalturaPluginWrapper(function() {
    mw.PluginManager.add('ChapterScrubberPlugin', mw.KBaseComponent.extend({
        ...
        setup: function (embedPlayer) {
            this.segmentStart = this.getConfig('segmentStart');
            this.segmentEnd = this.getConfig('segmentEnd');
        ...
        addBindings: function () {
            ...
            this.bind('seeked', function () {
                ...
                _this.getPlayer().seekPending = false;
            });
            this.bind('playerReady', function () {
                _this.getComponent().css('display', 'block');
            });
            this.bind('timeupdate', function () {
                _this.checkSegmentBounds();
            });
        ...
        checkSegmentBounds: function () {
            // When a seek is done, this function ensures it is set within our segment.
            // When a changeMedia occurs, Kaltura sets the time to 0. This function also resets it to
            // within our segment AND at the chosen seek time (e.g. DV switch).
            var player = this.getPlayer();
            var time = player.getPlayerElementTime();
            if (time < this.segmentStart) {
                var seekTime = player.seekPending ? player.seekPendingTime : this.segmentStart;
                player.sendNotification('doSeek', seekTime);
            }
            else if (time >= this.segmentEnd) {
                player.sendNotification('doStop');
            }
        },
        percentToUi: function (perc) {
            var segmentDuration = this.segmentEnd - this.segmentStart;
            var pos = perc * this.duration;
            var segmentPos = pos - this.segmentStart;
            return Math.round(segmentPos / segmentDuration * 1000);
        },
        uiToPercent: function (val) {
            var segmentDuration = this.segmentEnd - this.segmentStart;
            var segmentPos = val / 1000 * segmentDuration;
            var pos = segmentPos + this.segmentStart;
            return pos / this.duration;
        },
        updatePlayheadPercentUI: function (perc) {
            if (this.updateEnabled) {
                this.updatePlayheadUI(this.percentToUi(perc));
            }
        },
        ...
        showThumbnailPreview: function (data) {
            ...
            var perc = this.uiToPercent(data.val);
            perc = perc > 1 ? 1 : perc;
            ...
            if( this.embedPlayer.isDVR() ){
                ...
                timeText = kWidget.seconds2npt(currentTime - this.segmentStart);
            }
            ...
            return {
                ...
                change: function (event, ui) {
                    ...
                    if (embedPlayer.userSlide) {
                        var seekTime = _this.uiToPercent(ui.value) * _this.duration;
                        embedPlayer.userSlide = false;
                        embedPlayer.seeking = true;
                        embedPlayer.triggerHelper("userInitiatedSeek", seekTime);
                        embedPlayer.seekPending = true;
                        embedPlayer.seekPendingTime = seekTime;
